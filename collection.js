...
function handleCSVUpload(event) {
    ...
    Papa.parse(file, {
        ...
        complete: function(results) {
            ...
            pendingImports = results.data.map((row, index) => {
                ...
                const medianVal = getValue('Median', 'median', 'Median Price', 'median_price');
                const lowVal = getValue('Low', 'low', 'Min', 'min', 'Lowest', 'lowest');
                const highVal = getValue('High', 'high', 'Max', 'max', 'Highest', 'highest');
                const lastSoldVal = getValue('Last Sold', 'last_sold', 'Sold', 'sold_price');
                ...
                const purchasePriceVal = getValue('Purchase Price', 'purchase_price', 'Price', 'price', 'Paid', 'paid');
                const purchasePrice = extractPrice(purchasePriceVal) || 0;

                return {
                    discogsId: getValue('Catalog#', 'Catalog #', 'catalog', 'Catalog Number', 'Cat No', 'catno'),
                    artist: artist || 'Unknown Artist',
                    title: title || 'Unknown Title',
                    label: getValue('Label', 'label', 'LABEL'),
                    catalogueNumber: getValue('Catalog#', 'Catalog #', 'catalog', 'Catalog Number', 'Cat No', 'catno'),
                    ...
                    csvMarketData: Object.keys(csvMarketData).length > 0 ? csvMarketData : null
                };
...
function showCSVPreview() {
    ...
    tbody.innerHTML = pendingImports.slice(0, 10).map(r => {
...
function startPhotoVerification() {
    if (currentVerifyIndex >= pendingImports.length) {
        ...
        showToast(`Import complete! ${importedCount} records added to your collection.`, 'success');
        return;
    }
...
function showPhotoVerifyModal(record) {
    ...
    document.getElementById('verifyRecordName').textContent = `${record.artist} - ${record.title}`;
    
    // Pre-fill with Discogs data where available
    document.getElementById('verifyPurchasePrice').value = '';
    document.getElementById('verifyPurchaseDate').value = new Date().toISOString().split('T')[0];
    document.getElementById('verifySource').value = 'discogs';
    document.getElementById('verifyNotes').value = `Imported from Discogs. Format: ${record.format}${record.label ? ', Label: ' + record.label : ''}`;
...
function renderVerifyPhotos() {
    ...
    grid.innerHTML = verifyPhotos.map((file, idx) => `
...
const record = pendingImports[currentVerifyIndex];
    ...
    const userPrice = parseFloat(document.getElementById('verifyPurchasePrice').value);
    record.purchasePrice = userPrice || record.purchasePrice || 0;
    record.purchaseDate = document.getElementById('verifyPurchaseDate').value || record.purchaseDate;
    record.purchaseSource = document.getElementById('verifySource').value || record.purchaseSource;
    record.conditionVinyl = document.getElementById('verifyVinylCondition').value;
    record.conditionSleeve = document.getElementById('verifySleeveCondition').value;
    
    // Append user notes to existing notes
    const userNotes = document.getElementById('verifyNotes').value;
    if (userNotes) {
        record.notes = userNotes + (record.matrixNotes ? '\n\nMatrix/Notes: ' + record.matrixNotes : '');
    }
    ...
    if (localStorage.getItem('imgbb_api_key') && verifyPhotos.length > 0) {
        ...
        record.photos = await Promise.all(verifyPhotos.map(async (file) => {
    ...
    const purchaseDate = record.purchaseDate ? new Date(record.purchaseDate) : new Date();
    record.daysOwned = Math.floor((new Date() - purchaseDate) / (1000 * 60 * 60 * 24));
    ...
    try {
        ...
        const baseValue = record.csvMarketData?.median || record.purchasePrice || 10;
        ...
        record.roi = record.purchasePrice > 0 ? ((record.profitPotential / record.purchasePrice) * 100).toFixed(1) : '0';
        record.ebayStrategy = generateEbayStrategy(record);
    }

    // Mark as enriched if we have good data
    record.enrichmentStatus = record.marketData?.lastSold?.length > 0 ? 'complete' : 'partial';
...
async function analyzeRecordForResale(record) {
    ...
    if (window.discogsService?.key) {
        try {
            const search = await window.discogsService.searchRelease(record.artist, record.title, record.catalogueNumber);
            if (search) {
    ...
    const hasAI = aiProvider === 'deepseek' ? 
    ...
    if (record.csvMarketData?.median && (!marketData.medianPrice || marketData.source === 'estimated')) {
        marketData = {
            ...marketData,
        ...
        const response = await fetch(
            provider === 'deepseek' ? 'https://api.deepseek.com/v1/chat/completions' : 'https://api.openai.com/v1/chat/completions',
            {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        { role: 'system', content: 'You are a vinyl record market expert with access to sales data. Be precise with pricing.' },
...
async function extractMarketData(releaseDetails, record) {
    ...
    const highPrice = releaseDetails.highest_price || (lowestPrice ? lowestPrice * 1.5 : null);
...
function generateEstimatedMarketData(record) {
    ...
    const yearMultiplier = record.year && record.year < 1980 ? 1.5 : 
                          record.year && record.year < 1990 ? 1.2 : 1;
    
    // Adjust for format
    const formatMultiplier = record.format?.includes('7"') ? 0.6 :
                            record.format?.includes('12"') ? 0.8 : 1;
...
function calculateEstimatedValue(record, marketData) {
    ...
    if (marketData.lastSold?.length > 0) {
        ...
        const sortedPrices = marketData.lastSold
            .map(s => s.price * (marketData.gradeAdjustment?.[record.conditionVinyl] || 1))
            .sort((a, b) => a - b);
        baseValue = sortedPrices[Math.floor(sortedPrices.length / 2)];
    } else if (marketData.medianSold) {
        // Use AI-provided median sold
        baseValue = marketData.medianSold * (marketData.gradeAdjustment?.[record.conditionVinyl] || 1);
    } else {
...
function calculateListingPrice(record) {
    ...
    const demandBoost = record.marketData?.demandScore > 2 || record.marketData?.rarityScore?.includes('rare') ? 1.2 : 1;
    
    // Trend adjustment
    const trendMult = record.marketData?.demandTrend === 'rising' ? 1.1 :
                      record.marketData?.demandTrend === 'falling' ? 0.9 : 1;
    
    return Math.round(Math.min(floorPrice * urgencyDiscount * demandBoost * trendMult, marketValue * 1.25));
}
...
function renderCollection() {
    ...
    grid.innerHTML = filtered.map((record, idx) => {
        const originalIdx = collection.indexOf(record);
        const profitClass = (record.profitPotential || 0) >= 0 ? 'text-profit' : 'text-loss';
        const profitIcon = (record.profitPotential || 0) >= 0 ? 'trending-up' : 'trending-down';
        const statusColors = {
            'owned': 'bg-gray-500',
            'listed': 'bg-blue-500',
            'sold': 'bg-green-500'
        ...
        const needsUpdate = record.needsEnrichment || !record.marketData?.lastSold?.length;
        ...
        if (needsUpdate && !hasCsvData) {
            valueDisplay = '<span class="text-yellow-400 text-xs">Please Update</span>';
            profitDisplay = '<span class="text-yellow-400 text-xs">Please Update</span>';
        } else {
            const estValue = record.estimatedValue || record.csvMarketData?.median || 0;
            valueDisplay = `£${estValue.toFixed(2)}`;
            const profit = record.profitPotential || (estValue - (record.purchasePrice || 0));
            const roi = record.roi || (record.purchasePrice > 0 ? ((profit / record.purchasePrice) * 100).toFixed(1) : 0);
            profitDisplay = `
...
function getFilteredCollection() {
    ...
    filtered.sort((a, b) => {
        switch(sortBy) {
            ...
            case 'purchasePrice': return (b.purchasePrice || 0) - (a.purchasePrice || 0);
            case 'estValue': return (b.estimatedValue || 0) - (a.estimatedValue || 0);
            case 'profit': return (b.profitPotential || 0) - (a.profitPotential || 0);
            case 'dateAdded':
...
function updatePortfolioStats() {
    ...
    const totalValue = collection.reduce((sum, r) => sum + (parseFloat(r.estimatedValue) || parseFloat(r.csvMarketData?.median) || 0), 0);
    ...
    statCards.forEach(card => {
        ...
        if (label === 'Total Records') {
            ...
            card.setAttribute('trend', roi >= 0 ? 'unrealized gain' : 'unrealized loss');
        }
    });
    ...
    if (totalRecordsEl && totalRecordsEl.hasAttribute && totalRecordsEl.setAttribute) {
        totalRecordsEl.setAttribute('value', totalRecords.toLocaleString());
    }
    if (totalInvestedEl && totalInvestedEl.hasAttribute && totalInvestedEl.setAttribute) {
    ...
    if (returnEl && returnEl.hasAttribute && returnEl.setAttribute) {
        ...
        returnEl.setAttribute('trend', roi >= 0 ? 'unrealized gain' : 'unrealized loss');
    }
}
// Record Actions
    ...
    if (record.marketData?.lastSold?.length > 0) {
        marketDataHtml = `
            <div class="bg-surface p-3 rounded-lg mb-3">
                ...
                <div class="space-y-1">
                    ${record.marketData.lastSold.map(sale => `
...
function markAsSold(index) {
    const soldPrice = prompt('Enter final sold price (£):', collection[index].suggestedListingPrice);
    if (soldPrice) {
        ...
        collection[index].actualProfit = collection[index].soldPrice - collection[index].purchasePrice - collection[index].fees;
        saveCollection();
        renderCollection();
        updatePortfolioStats();
        closeRecordModal();
...
async function uploadPhotosToImgBB(files) {
    ...
    if (!apiKey) {
        ...
        return await Promise.all(files.map(async (file) => await fileToBase64(file)));
    }
    ...
    return uploaded.length > 0 ? uploaded : await Promise.all(files.map(async (file) => await fileToBase64(file)));
}
