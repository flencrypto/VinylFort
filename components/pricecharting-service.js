...
class PriceChartingService {
  ...
  async searchGame(title, console) {
    try {
      const response = await fetch(
        `${this.baseUrl}/product?t=${encodeURIComponent(title)}&c=${encodeURIComponent(console)}`,
      );
  ...
  async searchVinyl(artist, title, catalogNumber) {
    ...
    const query = encodeURIComponent(
      `${artist} ${title} ${catalogNumber || ""} vinyl`.trim(),
    );
  ...
  async fetchDiscogsMarketData(artist, title, catalogNumber) {
    ...
    try {
      ...
      return {
        ...
        identifiers: details.identifiers?.map((i) => ({
          type: i.type,
          value: i.value,
        })),
        companies: details.companies?.map((c) => ({
          name: c.name,
  ...
  async fetchEbaySoldData(artist, title, catalogNumber) {
    ...
    try {
      ...
      const cacheKey = `ebay_${artist}_${title}_${catalogNumber}`.replace(
        /[^a-z0-9]/gi,
        "_",
      );
  ...
  aggregateMarketData(sources) {
    ...
    if (
      result.sources.includes("discogs") &&
      result.pricing?.currentListings?.count > 5
    ) {
  ...
  calculateValuation(recordData, marketData, userCondition) {
    ...
    if (marketData.unified.communityData) {
      ...
      if (ratio > 2)
        demandFactor = 1.3; // High demand
      else if (ratio > 1)
        demandFactor = 1.15; // Above average
    ...
    if (listingCount !== undefined) {
      if (listingCount < 3)
        rarityFactor = 1.4; // Very rare
      else if (listingCount < 10)
        rarityFactor = 1.2; // Uncommon
    ...
    const adjustedValue =
      baseValue * conditionAdjust * demandFactor * rarityFactor * vintageFactor;

    // Calculate confidence interval
    const volatility = marketData.pricing.currentListings
      ? (marketData.pricing.currentListings.highest -
          marketData.pricing.currentListings.lowest) /
